...
public class DoTwoCloseFar extends AbFunctions
{
	 ...
	 * @param depth
	 */
	public DoTwoCloseFar(int depth)
	{
	 ...
	 * @param sensorTwo
	 */
	public DoTwoCloseFar(int sensorOne, int sensorTwo)
	{
		numSensors = 2;
		sensorNums = new int[numSensors];	
	...
	/**
	 * Does a simplified printing of the DoTwoCloseFar AbFunctions
	...
	{
		testTabs(num);
		System.out.println("Do Two Close Far");
	...
	/**
	 * Saves a DoTwoCloseFar function to the file
	 * DoTwoCloseFar has a code of 5
	...
	protected void save(PrintWriter out) throws FileNotFoundException
	{
		...
		out.printf("%d, %d, %d\n", 5, sensorNums[0], sensorNums[1]);
		
		//save the left branch before the right
		leftChild.save(out);
		rightChild.save(out);
	}
